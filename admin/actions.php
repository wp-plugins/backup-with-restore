...
add_action( 'load-toplevel_page_' . HMBKP_PLUGIN_SLUG, 'hmbkp_request_delete_backup' );
...
add_action( 'load-toplevel_page_' . HMBKP_PLUGIN_SLUG, 'hmbkp_request_delete_schedule' );
...
add_action( 'load-toplevel_page_' . HMBKP_PLUGIN_SLUG, 'hmbkp_request_download_backup' );
...
add_action( 'load-toplevel_page_' . HMBKP_PLUGIN_SLUG, 'hmbkp_request_cancel_backup' );
...
function hmbkp_request_restore_backup() {
	...
	require_once( HMBKP_PLUGIN_PATH . '/hm-backup/class-wpbp-restore.php' );

	$restore = new Wpbp_Restore();

	$restore->start( $_GET['hmbkp_schedule_id'], $_GET['hmbkp_restore_backup'] );

	$url = add_query_arg( array( 'restore' => 'success' ) );
	
	wp_redirect( remove_query_arg( array( 'hmbkp_restore_backup', '_wpnonce' ), $url ), 303 );
...
add_action( 'load-toplevel_page_' . HMBKP_PLUGIN_SLUG, 'hmbkp_request_restore_backup' );
...
function hmbkp_ajax_cron_test() {
	...
	if ( is_wp_error( $response ) )
		echo '<div id="hmbkp-warning" class="updated fade"><p><strong>' . __( 'WP Backup has detected a problem.', 'hmbkp' ) . '</strong> ' . sprintf( __( '%1$s is returning a %2$s response which could mean cron jobs aren\'t getting fired properly. WP Backup relies on wp-cron to run scheduled back ups. See the %3$s for more details.', 'hmbkp' ), '<code>wp-cron.php</code>', '<code>' . $response->get_error_message() . '</code>', '<a href="http://wordpress.org/extend/plugins/WP Backup/faq/">FAQ</a>' ) . '</p></div>';
