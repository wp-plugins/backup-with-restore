jQuery( document ).ready( function( $ ) {
	...
	// Show restore confirm message for restoring a backup
	$( document ).on( 'click', '.hmbkp_manage_backups_row .restore-action', function( e ) {
		
		if ( ! confirm( hmbkp.restore_backup ) )
			e.preventDefault();
		...
		$.get(
			...
			function( data ) {
				...
				if ( ! data || data == 0 ) {
					...
					if ( isNewSchedule )
						location.replace( '//' + location.host + location.pathname  + '?page=simplebackup&hmbkp_schedule_id=' + scheduleId );
